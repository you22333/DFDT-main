...
class SwinTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
        ...
        # --- 修改点 1: 添加 final_map_H, W 计算 ---
        if self.num_layers > 0:
            downsample_factor = (2 ** (self.num_layers - 1))  # 例如 2**(1-1) = 1
            self.final_map_H = self.patches_resolution[0] // downsample_factor  # 例如 64 // 1 = 64
            self.final_map_W = self.patches_resolution[1] // downsample_factor  # 例如 64 // 1 = 64
        else:
            self.final_map_H = self.patches_resolution[0]
            self.final_map_W = self.patches_resolution[1]
        # --- 修改点 1 结束 ---
        ...
        # --- 修改点 2: 修改 self.conv 输出通道 ---
        # self.conv = nn.Conv2d(self.num_features, 2048, 1, 1) # 原来的代码 (或者类似)
        self.conv = nn.Conv2d(self.num_features, 512, kernel_size=1, stride=1)  # 修改为输出 512 通道
        # --- 修改点 2 结束 ---
        # self.conv = nn.Conv2d(1024, 2048, 1, 1)
    ...
    def forward_features(self, x):
        ...
        x = self.norm(x)  # B L C[16,49,1024]
        B = x.shape[0]
        C = x.shape[2]
        x = x.transpose(1, 2)
        # x = x.reshape(B, C, 7, 7)
        x = x.reshape(B, C, self.final_map_H, self.final_map_W)
    ...
    def forward(self, x):
        x = self.forward_features(x)  # [16,1024]
        # x = self.head(x)  # [16,1000]
        x = self.conv(x)
        return x
...
def load_pretrained(config, model):
    print(f">>>>>>>>>> Fine-tuned from {config} ..........")
    ...
    print(f">>>>>>>>>> loaded successfully '{config}'")
...
if __name__ == '__main__':
    ...
    model = SwinTransformer( img_size=256,         # 修改: 匹配输入图像 H, W
